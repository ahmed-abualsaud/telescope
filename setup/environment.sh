...
git clone --recurse-submodules https://github.com/telescope/telescope.git
(cd telescope/telescope-web && git checkout master)
(cd telescope && ./gradlew build)
...
(cd telescope/telescope-web && ./tools/package.sh)
